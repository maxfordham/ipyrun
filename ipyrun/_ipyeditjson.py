...
class EditListOfDicts():
    ...
    @property
    def li_values(self):
        return [w.widget_row.children[1].children[0].value for w in self.widgets] #  widget_only

    @li_values.setter
    def li_values(self, li_values):
        for n in range(0,len(li_values)):
            self.widgets[n].widget_row.children[1].children[0].value = li_values[n] #  widget_only
    ...
    def _update_change(self, change):
        self.li = [] # overwriting the list each time feels like overkill but otherwise it can handle loading in a new list from file. there is undoubteadly a more elegant approach...
...
class EditJson(EditListOfDicts, FileConfigController):
    ...
    def _update_from_file(self):
        self.li_values = [l['value'] for l in self.li]
    ...
    def display(self):
        # the file save and load dialogue
        box = widgets.VBox([
            self.button_bar,
            self.temp_message,
            self.inputform,
...
if __name__ =='__main__':
    ...
    # Example6
    fdir = r'appdata\inputs'
    editmfjson = EditMfJson(fdir)
    # display
